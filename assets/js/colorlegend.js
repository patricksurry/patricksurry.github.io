...
var colorlegend = function (target, scale, type, options) {
  ...
  var scaleTypes = ['linear', 'quantile', 'ordinal', 'threshold', 'quantize']
    , found = false
    , opts = options || {}
...
*/
    ...
    , x = opts.x || 0
    , y = opts.y || 0
    , fmt = opts.tickFormat || (function (v) { return v; })
//    , padding = [2, 4, 10, 4]               // top, right, bottom, left
  ...
  if (type === 'quantile') {
  ...
  else if (type === 'ordinal' || type === 'threshold' || type === 'quantize') {
    ...
    for (i = 0; i < linearBoxes ; i++) {
      colors[i] = scale(min + (i+0.5) * ((max - min) / linearBoxes));
    }
  }
/*
...
*/
  ...
  legendBoxes.append('text')
      ...
      .attr('dy', '.71em')
      .attr('x', function (d, i) {
        return (i + (type === 'threshold' ? 1 : 0)) * (boxWidth + boxSpacing) + (type === 'ordinal' || type == 'threshold' ? 0 : (boxWidth / 2));
      })
      .attr('y', function () {
        return boxHeight + 2;
      })
      ...
      .text(function (d, i) {
        ...
        if (type === 'ordinal') {
          return fmt(domain[i]);
        } else if (type === 'threshold' && i < colors.length) {
          return fmt(domain[i]);
        }
  ...
  if (title) {
    legend.append('text')
        ...
        .attr('x', (colors.length * ((boxWidth + boxSpacing) / 2)))
